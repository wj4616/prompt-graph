...
description: "Graph-of-Thought prompt enhancement skill (v2). Up to 29 active nodes (N01-N20 + N27-N34 + N35; N21-N26 deferred) across up to 17 wave labels (Wave 0 through Wave 9, plus Wave 1.5 N35 ComplexityAssessment and Wave 4.5a-d sub-waves) in 5 modes (minimal/normal/deep/verbose/deep-verbose) with orthogonal flags (--quiet, --strict-verify, --strict-verify=full, --learn, --o7-threshold N). Two independent axes: depth (minimal/normal/deep) × passes (single/verbose). Wave-modular orchestrator: inline role-switched analysis + ideation; KB-augmented synthesis spawn in deep/verbose modes; orchestrator-inline parallel verification (N14/N15/N16) — or agent-separated N16 under --strict-verify, or N14+N15+N16 cluster under --strict-verify=full (HC-23 single-response_id); true GoT multi-path synthesis with meta-aggregation (N27-N33) in verbose modes; anti-fragility adversarial hardening (N34) with 5 attack vectors in deep/verbose; 3-tier KB integration (embedded snippets in modules/kb-snippets.md + Dify MCP runtime queries); SendMessage-first conditional back-edge repair with typed routing. Default budget ≤2 spawns (≤1 on Claude Code via SendMessage repair); --strict-verify lifts to ≤3 (single-pass modes) or ≤7 (two-pass verbose modes); --strict-verify=full lifts to ≤5 single-pass / ≤7 two-pass; verbose modes 3-4 baseline parallel (wall-clock ≈ 2 spawns; user-perceived 3-5 min typical). Supports --minimal, --deep, --verbose, --quiet, --strict-verify[=full], --learn, --o7-threshold N; --spec/--plan are deferred (hard halt) pending the separate v2 spec/plan implementation plan. Outputs enhanced prompt in --- delimiters; offers save to ~/docs/epiphany/prompts/."
...
| `/prompt-graph-v2 --strict-verify=full` | Agent-separated N14 PreservationVerifier + N15 SemanticFidelityChecker + N16 QualityGate (W1 — workstream 1). Spawn budget ≤5 (single-pass; verifier-cluster shares budget in verbose tail). HC-23 single-response_id parallel dispatch. Orthogonal. |
| `/prompt-graph-v2 --learn` | Post-run topology_adjustments YAML written to Memory store at N19 SaveHandler. Orthogonal — combines with any mode and with `--quiet`/`--strict-verify`. Memory unavailable → silent skip per AP-V6. |
| `/prompt-graph-v2 --o7-threshold N` | Integer override for the O7 per-agent context-pressure threshold (default 24,000 tokens). Consumed by N35 ComplexityAssessment's verbose_pressure mutation rule. Invalid/missing integer → fall back to 24000. |
...
|---|---|---|---|---|
| **minimal** | `--minimal` | N01–N04, N35, N09, N11, N13–N19 (14 nodes — Wave 1.5 N35 active in all modes per v2 W2) | 0 | 1 synthesis + 1 optional repair = ≤2 (1 if SendMessage repair per O12) |
| **normal** | (no flag) | N01–N09, N35, N11–N19 (20 nodes; N10/N20/N27–N34 skipped) | 0 | 1 synthesis + 1 optional repair = ≤2 (1 if SendMessage repair per O12) |
| **deep** | `--deep` | N01–N09, N35, N10–N19, N34 (21 nodes; N20/N27–N33 skipped — single pass, anti-fragility tail) | Tier 2 (non-blocking MCP): dify-thought-kb, dify-cognitive-kb at N13 deep-mode augmentation | 1 synthesis + 1 optional repair = ≤2 |
| **verbose** | `--verbose` | N01–N20, N35, N27–N34 (29 nodes — normal path + multi-pass tail) | Tier 2 (non-blocking MCP): dify-thought-kb for topology, dify-cognitive-kb for trait at N27 | 1 baseline + 2–3 parallel + 1 optional repair = ≤5 spawns (3–4 without repair; wall-clock ≈ 2) |
| **deep-verbose** | `--deep --verbose` | All 35 nodes (N01–N35) | Tier 2 at both N13 (deep augmentation) AND N27 (branch routing) | 1 baseline + 2–3 parallel + 1 optional repair = ≤5 spawns (3–4 without repair; wall-clock ≈ 2) |
...
| **strict-verify-full** | `--strict-verify=full` (combines with any) | Same as combined mode + N14/N15/N16 all spawn at Wave 5 under HC-23 single-response_id dispatch | Same as combined mode | +3 spawns per Wave-5 verifier cluster firing (N14+N15+N16 as parallel Agent calls sharing one response_id). Single-pass modes: ≤5 (N13 + 3 verifiers + 1 optional repair). Two-pass modes: in the verbose tail (Wave 8) the verifier cluster falls back to bare `--strict-verify` semantics (N16-only) to stay within the ≤7 cap — so worst case ≤7 (N13 + 3 Wave-5 verifiers + 1 Wave-8 N16 + 2–3 parallel + 1 repair, with the parallel and repair budget shared against the verifier cluster). **(NEW v2 — workstream 1)** |
...
All paths below include Wave 1.5 (N35 ComplexityAssessment, NEW v2 W2) between Wave 1 and Wave 2/3. N35 is inline (no spawn budget consumption); see `modules/m-wave1.5-complexity.md` for protocol. Node counts include N35.

- **Minimal path** — triggered by `--minimal` flag. 14 nodes: N01–N04 + N35 (Wave 1.5) + N09, N11, N13–N19. Skips Wave 2 (analysis), N10 (anti-conformity), N12 (coherence), N20, N27–N34. 1 spawn + optional 1 repair.
- **Normal path** — default (no flag). 20 nodes (N01–N09 + N35 + N11–N19). Skips N20 and N27–N34. Merged 2-block analysis. No anti-conformity (N10 skipped), no anti-fragility. 1 spawn + optional 1 repair.
- **Deep path** — triggered by `--deep`. 21 nodes: normal set + N10 (anti-conformity) + N34 (anti-fragility). N13 is KB-augmented (Snippets 5–6 + cognitive trait protocol). N34 receives N13 output directly via E91 (single-agent path — no multi-path layer). Skips N20, N27–N33. 1 spawn + optional 1 repair.
- **Verbose path** — triggered by `--verbose`. 29 nodes: normal path through Wave 4 (N13 first-pass baseline) then Waves 4.5a–d (N27→N28–N32 parallel→N33→N34)→Wave 7 (N20 expansion)→Wave 8 (re-verify)→Wave 9 (final router with revert-to-first-pass). No anti-conformity in first pass. 3–4 spawns (1 baseline + 2–3 parallel + optional repair; wall-clock ≈ 2).
- **Deep-verbose path** — triggered by `--deep --verbose`. All 35 nodes: deep path through Wave 4 (with N10 + KB-augmented N13) then verbose multi-path tail (N27→N28–N32→N33→N34→N20→re-verify→final router). N32 always selected per N27 deep-verbose rule. Maximum quality, maximum cost.
...
- `--strict-verify` → spawns N16 QualityGate as a separate Agent in Wave 5 (and Wave 8 for verbose/deep-verbose), realizing the Intuition-Verification Partnership. Lifts spawn budget cap by 1. Default OFF — opt-in only.
- `--strict-verify=full` → extends `--strict-verify` to spawn N14 PreservationVerifier and N15 SemanticFidelityChecker alongside N16 at Wave 5, under HC-23 single-response_id parallel dispatch (workstream 1). Lifts spawn cap to ≤5 in single-pass modes. In verbose tail (Wave 8), the verifier cluster falls back to bare `--strict-verify` semantics (N16-only) to stay within the ≤7 cap. See Section 3 row "strict-verify-full".
- `--learn` → at N19 SaveHandler, writes a `topology_adjustments` YAML to the resolved Memory directory (workstream 3). Off by default; opt-in only.
- `--o7-threshold N` → integer override for the O7 per-agent context-pressure threshold (default 24,000 tokens; spec calibration constant). Consumed by N35 ComplexityAssessment's `verbose_pressure` mutation rule.
All orthogonal flags combine with each other and with any mode flag.
