...
- `expansion_completed: bool` — initialized to false; set to true at start of Wave 9 in verbose or deep-verbose mode
...
2. **Classify failure family (O13 typed-repair-routing — v2 7-check schema):**
   - **Family-A (preservation):** failing_checks ⊆ {6a, 6b}. Two distinct sub-cases:
     - **6a-only (verbatim items missing):** root cause is INVENTORY omission. Replay N04 (inline) with "missed item" hint, then N09 → N11 (inline) to refresh contracts, then resume/respawn N13. family_hint: "Items missing verbatim — re-emit each in the semantically appropriate XML section per S1 placement mapping."
     - **6b-only (items present but mis-placed):** root cause is N13's S1 placement step, NOT INVENTORY. **SKIP N04 replay** (INVENTORY already correct — N04 emits YAML keys, not XML sections). Instead replay N11 (inline) to inject a placement-correction contract: `{technique: T1, target_section: <correct>, action: "Move INVENTORY item '[X]' from <wrong> to <correct> per S1", priority: high, source_weakness: null}`. Then resume/respawn N13. family_hint: "Items present but in wrong XML section — relocate per added placement contract; do not re-emit verbatim."
     - **Mixed 6a+6b:** run both replays in order — N04 re-extraction first, then N11 placement-correction.
   ...
   - **Family-C (quality):** failing_checks ⊆ {6h, 6i, 6j, 6l}. Root cause is genuine synthesis quality. Repair target: resume/respawn N13 directly with the targeted check IDs in repair_signal.
...
3. **Route:**
   - IF failing_checks empty AND (mode NOT IN {verbose, deep-verbose} OR expansion_completed = true): → E20 route `{verified_xml, "verified", preservation_summary}` to N18 (PASS path; terminal).
   - IF failing_checks empty AND mode IN {verbose, deep-verbose} AND expansion_completed = false: → E22 route first_pass_verified_xml to N20 (expansion wave); retain first_pass_verified_xml as N17 internal state (for potential Wave 9 revert).
...
**Verbose / deep-verbose repair routing (NB1 resolution):** In single-pass modes (minimal/normal/deep), the verified XML originated from N13 (with optional N34 hardening in deep). Repair targets N13 directly. **In verbose / deep-verbose modes, the verified XML is the post-N34 hardening of N33's aggregated output of N28-N32 strategy agents — N13's first-pass baseline is no longer the load-bearing artifact.** SendMessage-resume to N13 in those modes would resume the WRONG agent. Verbose-mode repair policy:
- **Default verbose-mode repair:** fresh-spawn N13 only (NOT SendMessage-resume). N13's repair output replaces the prior baseline; the multi-path tail (N27→N28-N32→N33→N34) does NOT re-run on repair (cap: ≤1 N13 fresh-spawn repair, no second multi-path pass per O6).
- **Optional multi-agent SendMessage path (when `multi_synthesis_agents` provenance pinpoints a single strategy):** if N33's aggregation provenance attributes the failing section(s) to exactly one strategy agent (N28-N32) AND that agent's id is captured, SendMessage-resume to that strategy agent with a section-scoped repair message. Otherwise fall back to fresh-spawn N13. Multi-agent SendMessage is opportunistic; the safe default is fresh-spawn N13.
- **Cap accounting:** fresh-spawn N13 in verbose modes consumes the single repair slot per O6 (≤1 repair attempt). Multi-agent SendMessage-resume also consumes the slot (counts the attempt, not the spawn).

**Step-by-step (single-pass modes — minimal/normal/deep):**
...
- `first_pass_verified_xml` — held when E22 fires for expansion-failure revert in Wave 9 (verbose, deep-verbose).
