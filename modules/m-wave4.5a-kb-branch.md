# Wave 4.5a Module — KB Branch Routing (N27)
...
**MCP query timeout (DF2):** Each MCP query has a hard wall-clock cap of **5 seconds**. The orchestrator sets a 5s timer when the query fires and proceeds to the heuristic if the response has not arrived by deadline. Any response arriving after the deadline is discarded. If two queries fire in the same wave (e.g., dify-thought-kb + dify-cognitive-kb in deep-verbose), they run concurrently — the deadline is per-query, not aggregate, so the wave proceeds after at most 5s of MCP wait time. Implementations on host runtimes that lack first-class timeout primitives MUST default to fire-and-forget (no wait) — never block indefinitely.

In deep-verbose mode, additionally query `mcp__dify-cognitive-kb__cognitive-research-kb-dify` for the cognitive trait best matching the input's thinking demands. Fallback to "Precision Forcing" (universal fallback trait) if query fails, returns empty, or exceeds the 5s deadline.
...
**Deep-verbose binding enforcement gate (DF4):** After branch selection completes, before emitting the branch plan, the orchestrator MUST run this validation:
```
if mode == "deep-verbose" and N32 not in selected_agents:
    if branch_width == 3:
        replace selected_agents[2] with N32 (drop the third-best-fit, keep top 2 + N32)
    elif branch_width == 2:
        replace selected_agents[1] with N32 (drop the second-best-fit, keep best-fit + N32)
    log "Deep-verbose binding: N32 inserted to satisfy always-selected rule;
         displaced [<original strategy>]"
```
This gate is non-bypassable in deep-verbose mode. Heuristic-based selection that excludes N32 is silently corrected before the branch plan emits.
...
4. Strategy-specific delta — appended after `=== CONTRACTS END ===`, from the Multi-Synthesis module (m-wave4.5b-multi-synthesis.md)

The assembled spawn prompt is passed to the multi-path synthesis layer via E81 (fan to selected N28-N32 agents). Per-agent assembly is documented in m-wave4.5b-multi-synthesis.md; N27's output is the branch plan + the routing decision. The actual spawns fire in Wave 4.5b.

**Per-agent O11 INVENTORY coverage check (DF5):** Before E81 fires, N27 runs the O11 grep over **each** assembled per-agent spawn prompt independently (not just the shared base content). Strategy deltas differ across N28-N32, and INVENTORY items referenced by the base contracts may end up unreferenced after the delta is appended for a given strategy (e.g., a CreativeDC delta that emphasizes structural exploration may not echo every key_constraints item). For each agent's prompt, for each non-empty INVENTORY key:
- if at least one item is grep-findable in that agent's assembled prompt → PASS for that key for that agent.
- if no item is grep-findable → append a per-agent `=== PRESERVE-VERBATIM RIDER ===` block at the end of that agent's prompt, listing the missing items. Same one-line preservation reminder as N13's O11 rider.

This is non-blocking — riders attach automatically; PG5 spawn proceeds. The check pre-empts a class of preservation failures that would otherwise bubble to N33 aggregation as content gaps.
