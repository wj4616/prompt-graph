...
This gate is independent of `--strict-verify=full` — N34 spawn fires whenever INVENTORY exceeds the threshold, in any mode where N34 itself is active (deep / verbose / deep-verbose). The signal source is N35 ComplexityAssessment's `mode_mutation_signal: promote_n34_to_spawn` emitted via E53 (W2 / Phase 3); N34 consumes the signal here.
